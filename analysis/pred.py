...
def build_markov_model(logfile_ref, context_size):
    ...
    with LogFileRead(logfile_ref) as f:
        for access in f:
            ...
            state = (access.page_index, access.pid_self, access.pid_next)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cache-size', type=int, default=0, help='size of the cache to simulate (0 for unlimited)')
    parser.add_argument('--lookahead-size', type=int, default=1, help='number of future accesses to predict and include in the cache')
    parser.add_argument('--context-size', type=int, default=3, help='number of past accesses to include in the state for the Markov model')
