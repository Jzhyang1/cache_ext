...
void BPF_STRUCT_OPS(pf_prefetch_folios, struct cache_ext_prefetch_ctx *eviction_ctx,
		    struct mem_cgroup *memcg)
{
	prefetch_count++;
	// don't do anything
	bpf_printk("cache_ext: prefetching...\n");
}

void BPF_STRUCT_OPS(pf_folio_accessed, struct folio *folio, bool *prefetch) {
	if (prefetch_count == 0) {
		*prefetch = true;
	}
}
